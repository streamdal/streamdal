        ...
        while (reader.pos < end) {
            ...
            switch (fieldNo) {
                case 1:
                    ...
                    break;
                case 2:
                    val = sp_common_4.Metric.internalBinaryRead(reader, reader.uint32(), options);
                    break;
                default: throw new globalThis.Error("unknown map entry field for field protos.GetMetricsResponse.metrics");
            }
        }
        map[key !== null && key !== void 0 ? key : ""] = val !== null && val !== void 0 ? val : sp_common_4.Metric.create();
    }
    internalBinaryWrite(message, writer, options) {
        ...
        for (let k of Object.keys(message.metrics)) {
            writer.tag(1, runtime_1.WireType.LengthDelimited).fork().tag(1, runtime_1.WireType.LengthDelimited).string(k);
            writer.tag(2, runtime_1.WireType.LengthDelimited).fork();
            sp_common_4.Metric.internalBinaryWrite(message.metrics[k], writer, options);
            writer.join().join();
        }
        let u = options.writeUnknownFields;
        if (u !== false)
