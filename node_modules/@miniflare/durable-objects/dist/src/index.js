...
var HEX_ID_REGEXP = /^[A-Fa-f0-9]{64}$/;
...
var DurableObjectStorage = class {
  ...
  #pendingFlushes = 0;
  #noPendingFlushesPromise;
  #noPendingFlushesPromiseResolve;
  async #runWrite(closure, options) {
    this.#pendingFlushes++;
    if (this.#noPendingFlushesPromise === void 0) {
      (0, import_assert3.default)(this.#noPendingFlushesPromiseResolve === void 0);
      this.#noPendingFlushesPromise = new Promise((resolve) => this.#noPendingFlushesPromiseResolve = resolve);
    }
    try {
      return await (0, import_shared4.waitUntilOnOutputGate)((0, import_shared4.runWithInputGateClosed)(closure, options?.allowConcurrency), options?.allowUnconfirmed);
    } finally {
      (0, import_assert3.default)(this.#pendingFlushes > 0);
      (0, import_assert3.default)(this.#noPendingFlushesPromiseResolve !== void 0);
      this.#pendingFlushes--;
      if (this.#pendingFlushes === 0) {
        this.#noPendingFlushesPromiseResolve();
        this.#noPendingFlushesPromiseResolve = void 0;
        this.#noPendingFlushesPromise = void 0;
      }
    }
  }
  ...
  async deleteAll(options) {
    return this.#runWrite(() => this.#mutex.runWithWrite(async () => {
...
var DurableObjectsPlugin = class extends import_shared5.Plugin {
  ...
  async getInstance(storage, id) {
    const state = await this.getObject(storage, id);
    return state[kInstance];
  }
