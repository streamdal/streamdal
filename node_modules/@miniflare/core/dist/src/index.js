...
__export(exports, {
  ...
  _isBodyStream: () => _isBodyStream,
  _isByteStream: () => _isByteStream,
  _isDisturbedStream: () => _isDisturbedStream,
  _isFixedLengthStream: () => _isFixedLengthStream,
  _kInner: () => _kInner,
...
function assertsInRequest(func, assert5) {
  return assert5 ? (...args) => {
...
// packages/core/src/standards/crypto.ts
var CryptoKey = globalThis.CryptoKey ?? import_crypto.webcrypto.CryptoKey;
...
var importKey = async function(format2, keyData, algorithm, extractable, keyUsages) {
  const forcePublic = format2 === "raw" && typeof algorithm === "object" && algorithm.name === "NODE-ED25519" && "namedCurve" in algorithm && algorithm.namedCurve === "NODE-ED25519";
  algorithm = await ensureValidNodeAlgorithm(algorithm);
  if (forcePublic)
    algorithm.public = true;
  const key = await import_crypto.webcrypto.subtle.importKey(format2, keyData, algorithm, extractable, keyUsages);
  return ensureValidWorkerKey(key);
};
var exportKey = async function(format2, key) {
  key = await ensureValidNodeKey(key);
  return import_crypto.webcrypto.subtle.exportKey(format2, key);
};
  ...
  const subtle = new Proxy(import_crypto.webcrypto.subtle, {
    get(target, propertyKey, receiver) {
      ...
      if (propertyKey === "timingSafeEqual")
        return import_crypto.timingSafeEqual;
...
var kBodyStreamBrand = Symbol("kBodyStreamBrand");
...
function _isDisturbedStream(stream) {
  for (const symbol of Object.getOwnPropertySymbols(stream)) {
    if (symbol.description === "kState") {
...
import_web2.ReadableStream.prototype.tee = function() {
  ...
  if (_isByteStream(this)) {
    stream1 = convertToByteStream(stream1, true);
    stream2 = convertToByteStream(stream2, true);
  }
  const branded = this;
...
function _isFixedLengthStream(stream) {
  return stream[kContentLength] !== void 0;
}
...
var CompressionStream = class extends import_web2.TransformStream {
  constructor(format2) {
    if (format2 !== "gzip" && format2 !== "deflate") {
      throw new TypeError("The compression format must be either 'deflate' or 'gzip'.");
    }
    const transform = format2 === "gzip" ? import_zlib.default.createGzip() : import_zlib.default.createDeflate();
...
var DecompressionStream = class extends import_web2.TransformStream {
  constructor(format2) {
    ...
    const transform = format2 === "gzip" ? import_zlib.default.createGunzip() : import_zlib.default.createInflate();
    super(createTransformerFromTransform(transform));
  }
};
...
var kBodyStream = Symbol("kBodyStream");
...
function _isBodyStream(stream) {
  return stream[kBodyStreamBrand] === true;
}
  ...
  constructor(input, init) {
    ...
    if (input instanceof import_undici.Request && !init) {
      ...
      if (init instanceof Request)
        init = init[_kInner];
      init = init ? cloneRequestInit(init) : void 0;
      if (init?.body instanceof ArrayBuffer) {
        init.body = init.body.slice(0);
      }
      if (init?.body instanceof import_web3.ReadableStream) {
        init.duplex = "half";
...
async function fetch(input, init) {
  ...
  const baseRes = await (0, import_undici.fetch)(req[_kInner], { dispatcher });
...
function parseRanges(rangeHeader, length) {
  ...
  for (const range of ranges) {
    const match2 = rangeRegexp.exec(range);
    if (match2 === null)
      return;
    const { start, end } = match2.groups;
...
function getRangeResponse(requestRangeHeader, responseStatus, responseHeaders, responseBody) {
  ...
  if (ranges === void 0) {
    ...
    const stream = new import_web4.ReadableStream({
      type: "bytes",
      pull(controller) {
...
var import_node_assert = __toModule(require("node:assert"));
var assert_default = import_node_assert.default;
var AssertionError = import_node_assert.default.AssertionError;
var deepEqual = import_node_assert.default.deepEqual;
var deepStrictEqual = import_node_assert.default.deepStrictEqual;
var doesNotMatch = import_node_assert.default.doesNotMatch;
var doesNotReject = import_node_assert.default.doesNotReject;
var doesNotThrow = import_node_assert.default.doesNotThrow;
var equal = import_node_assert.default.equal;
var fail = import_node_assert.default.fail;
var ifError = import_node_assert.default.ifError;
var match = import_node_assert.default.match;
var notDeepEqual = import_node_assert.default.notDeepEqual;
var notDeepStrictEqual = import_node_assert.default.notDeepStrictEqual;
var notEqual = import_node_assert.default.notEqual;
var notStrictEqual = import_node_assert.default.notStrictEqual;
var ok = import_node_assert.default.ok;
var rejects = import_node_assert.default.rejects;
var strict = import_node_assert.default.strict;
var strictEqual = import_node_assert.default.strictEqual;
var throws = import_node_assert.default.throws;
...
__export(async_hooks_exports, {
  ...
  AsyncResource: () => import_node_async_hooks.AsyncResource,
  default: () => async_hooks_default
});
var import_node_async_hooks = __toModule(require("node:async_hooks"));
...
__export(buffer_exports, {
  ...
  constants: () => import_node_buffer.constants,
  default: () => buffer_default,
  kMaxLength: () => import_node_buffer.kMaxLength,
  kStringMaxLength: () => import_node_buffer.kStringMaxLength
});
...
var import_node_events = __toModule(require("node:events"));
var events_default = import_node_events.default;
var EventEmitter = import_node_events.default.EventEmitter;
var EventEmitterAsyncResource = import_node_events.default.EventEmitterAsyncResource;
var captureRejectionSymbol = import_node_events.default.captureRejectionSymbol;
var defaultMaxListeners = import_node_events.default.defaultMaxListeners;
var errorMonitor = import_node_events.default.errorMonitor;
var getEventListeners = import_node_events.default.getEventListeners;
var listenerCount = import_node_events.default.listenerCount;
var on = import_node_events.default.on;
var once = import_node_events.default.once;
var setMaxListeners = import_node_events.default.setMaxListeners;

// packages/core/src/plugins/node/util.ts
var util_exports = {};
__export(util_exports, {
  _extend: () => import_node_util._extend,
  callbackify: () => import_node_util.callbackify,
  default: () => util_default,
  format: () => import_node_util.format,
  inherits: () => import_node_util.inherits,
  promisify: () => import_node_util.promisify,
  types: () => import_node_util.types
});
...
function additionalNodeModules(_experimental) {
  const modules = {
    "node:assert": assert_exports,
    "node:async_hooks": async_hooks_exports,
    "node:buffer": buffer_exports,
    "node:events": events_exports,
    "node:util": util_exports
  };
  return modules;
}
...
var CorePlugin = class extends import_shared10.Plugin {
  ...
  constructor(ctx, options) {
    ...
    let CompatReadableStream = import_web5.ReadableStream;
    let CompatWritableStream = import_web5.WritableStream;
    let CompatTransformStream = import_web5.TransformStream;
    if (!enableStreamConstructors) {
      CompatReadableStream = proxyDisableStreamConstructor(import_web5.ReadableStream);
      CompatWritableStream = proxyDisableStreamConstructor(import_web5.WritableStream);
      if (enableTransformStreamConstructor) {
        CompatTransformStream = proxyDisableStreamConstructor(import_web5.TransformStream);
      }
    }
    if (!enableTransformStreamConstructor) {
    ...
    if (enableStreamConstructors) {
      extraGlobals.ReadableByteStreamController = import_web5.ReadableByteStreamController;
      extraGlobals.ReadableStreamBYOBRequest = import_web5.ReadableStreamBYOBRequest;
      extraGlobals.ReadableStreamDefaultController = import_web5.ReadableStreamDefaultController;
      extraGlobals.WritableStreamDefaultController = import_web5.WritableStreamDefaultController;
      if (enableTransformStreamConstructor) {
        extraGlobals.TransformStreamDefaultController = import_web5.TransformStreamDefaultController;
      }
    }
    try {
    ...
    const CompressionStreamImpl = import_web6.default.CompressionStream ?? CompressionStream;
    const DecompressionStreamImpl = import_web6.default.DecompressionStream ?? DecompressionStream;
    ...
    this.#globals = {
      ...
      crypto,
      CryptoKey,
      TextDecoder: import_util2.TextDecoder,
      ...
      ReadableStreamBYOBReader: import_web5.ReadableStreamBYOBReader,
      ReadableStreamDefaultReader: import_web5.ReadableStreamDefaultReader,
      WritableStreamDefaultWriter: import_web5.WritableStreamDefaultWriter,
      ByteLengthQueuingStrategy: import_web5.ByteLengthQueuingStrategy,
      CountQueuingStrategy: import_web5.CountQueuingStrategy,
      IdentityTransformStream,
      FixedLengthStream,
      CompressionStream: CompressionStreamImpl,
      DecompressionStream: DecompressionStreamImpl,
      TextEncoderStream: import_web5.TextEncoderStream,
      TextDecoderStream: import_web5.TextDecoderStream,
      Event,
      EventTarget,
      AbortController,
      AbortSignal,
      ...
      DOMException: DOMException2,
      WorkerGlobalScope,
      structuredClone: globalThis.structuredClone ?? import_shared10.structuredCloneImpl,
...
function throwNoScriptError(modules) {
  ...
  const format2 = modules ? "modules" : "service-worker";
  ...
  if (modules) {
    lines.push("- Set build.upload.main in wrangler.toml", (0, import_colors2.dim)("    [build.upload]"), (0, import_colors2.dim)(`    format = "${format2}"`), (0, import_colors2.dim)(`    dir = "dist"`), (0, import_colors2.dim)(`    main = "${script}"`));
  }
  lines.push("");
  throw new MiniflareCoreError("ERR_NO_SCRIPT", lines.join("\n"));
}
var kParentSharedCache = Symbol("kParentSharedCache");
...
var MiniflareCore = class extends import_shared12.TypedEventTarget {
  ...
  get #isMount() {
    return this.#ctx[kParentSharedCache] !== void 0;
  }
  ...
  async #reload(dispatchReloadEvent = true) {
    ...
    if (script && (!this.#ctx.scriptRunForModuleExports || modules && requiresModuleExports)) {
      ...
      res = await this.#ctx.scriptRunner.run(globalScope, script, rules, additionalModules, this.#compat);
  ...
  async dispatchQueue(batch) {
    ...
    return new import_shared12.RequestContext({
      externalSubrequestLimit: (0, import_shared12.usageModelExternalSubrequestLimit)(usageModel)
    }).runWith(() => globalScope[kDispatchQueue](batch));
  }
  async dispose() {
    await this.#initPromise;
    for (const [name] of this.#plugins) {
