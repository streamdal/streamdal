...
export const Service = ({data}: { data: { label: string } }) => {
    return (
        <div class="h-[100px]">
            <div
                class="h-[80px] w-[320px] flex items-center justify-between bg-white rounded-lg shadow-lg z-10 border-1 border-purple-200 px-2">
                <IconGripVertical
...
export const Producer = ({data}: { data: ActorNodeData }) => {
    ...
    return (
        <div className="h-[96px] flex items-center z-40">
            ...
            {data.instances && (
                <div
                    class="absolute inline-flex items-center justify-evenly w-7 h-7 text-xs text-white bg-purple-500 rounded-full top-1 -right-2 dark:border-gray-900">
                    {data.instances}
                </div>
            )}
            {data.flow && <Handle
