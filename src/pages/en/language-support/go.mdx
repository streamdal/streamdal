---
title: "Go Client Support for Streamdal"
metaTitle: "Elevate Your Go Applications with Streamdal"
description: "Integrate Streamdal seamlessly into your Go projects and optimize your data operations."
layout: ../../../layouts/MainLayout.astro
---
import { Notification } from "accessible-astro-components";
import warning from "@images/icons/warning.png";

<br />
<br />
If you're developing with Go and aim to elevate your data operations, you're in the right place. With Streamdal's Go SDK, integrating the power of Streamdal into your Go applications has never been simpler.

<Notification type="warn">
  <img src={warning} alt="Warning" width="30px" height="30px" />
  <div class="prose-md prose-p:my-2 prose-a:text-purple-dark prose-a:underline">
    <p>For detailed information on the SDK and its functionalities, refer to the [SDK README](https://github.com/streamdal/go-sdk).</p>
  </div>
</Notification>

## Dive Into an Example

Wondering how it works in action? Here's a simple Go code demonstrating the Streamdal integration:

```go
package main

import (
	"context"
	"fmt"
	"time"

	"github.com/streamdal/go-sdk"
)

func main() {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	sc, err := streamdal.New(&streamdal.Config{
		ServerURL:       "localhost:8082",
		ServerToken:     "streamdal",
		StepTimeout:     time.Millisecond * 10,
		PipelineTimeout: time.Millisecond * 100,
		DryRun:          false,
		ServiceName:     "billing-svc",
		ShutdownCtx:     ctx,
	})
	if err != nil {
		panic(err)
	}

	resp, err := sc.Process(ctx, &streamdal.ProcessRequest{
		OperationType: streamdal.OperationTypeConsumer,
		OperationName: "new-order-topic",
		ComponentName: "kafka",
		Data:          []byte(`{"object": {"field": true}}`),
	})
	if err != nil {
		panic(err)
	}

	fmt.Printf("%#v\n", resp)
}
```
## Example Code Breakdown

**Imports:** The necessary Go packages, including the Streamdal SDK, are imported.

**Context Creation:** A context, allowing for timeouts and graceful shutdowns, is established.

**Streamdal Client Setup:** The SDK client is instantiated with configurations such as the server URL, service name, and timeouts.

**Error Checks:** If there's an initialization error, the program halts and displays the error.

**Data Processing:** The Process() method is called on the Streamdal client to process data, specifying details like operation type and the actual data.

**Display Output:** The processed data's response is printed to the console.